...
export default function FeaturedProducts() {
  ...
  const products = storeProducts && storeProducts.length > 0 ? storeProducts.slice(0, 8) : fallbackProducts;
  return (
    <section className="bg-black text-white py-20 relative">
      <div className="max-w- mx-auto px-6">
        {/* Shop By Brand moved to TopCategories */}

        {/* Explore Parts section (new) */}
        <div className="text-center my-8">
          <h2 className="text-4xl font-extrabold tracking-wide drop-shadow-xl">🔎 Explore Parts</h2>
          <p className="text-gray-400 mt-2">Browse popular categories and parts — open a part in the Live 3D editor</p>
        </div>

        <div className="relative mb-8">
          <div className="flex gap-6 sm:gap-8 overflow-x-auto snap-x snap-mandatory py-6 scrollbar-hide px-2">
            {(allProducts && allProducts.length > 0 ? allProducts.slice(0, 10) : fallbackProducts).map((p, i) => (
              <motion.article
                key={p.id ?? i}
                ...
                className="flex-shrink-0 w-48 sm:w-56 md:w-64 bg-white/6 backdrop-blur-lg border border-white/10 rounded-2xl p-4 shadow-md hover:shadow-lg/30 transition-transform snap-start"
              >
                <div className="w-full h-28 sm:h-32 flex items-center justify-center bg-black/10 rounded-lg overflow-hidden">
                  {p.images?.[0] || p.img ? (
                ...
                <h3 className="mt-3 text-md font-semibold line-clamp-2">{p.title}</h3>
                <p className="text-red-400 font-bold mt-1">${p.price}</p>

                <div className="mt-3 flex gap-2">
                  <button onClick={() => navigate(`/product/${p.id}`)} className="flex-1 py-2 bg-white/10 hover:bg-white/20 text-sm rounded-lg border border-white/10">Details</button>
                  <button onClick={() => navigate(`/live3d/${p.id}`)} className="w-10 h-10 rounded-lg bg-red-500 hover:bg-red-600 flex items-center justify-center text-white">3D</button>
                </div>
              </motion.article>
            ))}
          </div>
        ...
        {/* Featured Picks now below the brands */}
        <div className="text-center my-8">
          <h2 className="text-4xl font-extrabold tracking-wide drop-shadow-xl">🔥 Featured Picks</h2>
          <p className="text-gray-400 mt-2">Hand-picked upgrades for performance, safety and style</p>
        </div>
        ...
        {/* Brand modal handled in TopCategories now */}
      </div>
    </section>
  );
}
